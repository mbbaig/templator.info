	...
	$(document).ready(function () {
		...
		var data = [
			@foreach($tasks as $task)
				["{{ $task->title }}", {{ $task->duration }}, "{{ $task->predecessorids }}", "{{ $task->note }}", "{{ $task->summarytaskid }}"],
			@endforeach
		];
		...
		$('#save').click(function() {
			...
			if (validData){
				...
				$.each(data, function(key, value){
					...
					unformatedData[key] = [str, value[1], value[2], value[3], value[4]];
				});
				$.ajax({
					url: "/tasks/update/{{ $template->id }}",
